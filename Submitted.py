...
def submitted(studentName,quizCode):

    score=db.child("Quiz Results").child(quizCode).child(studentName).get()
    score=int(score.val()['Correct Answers'])
    ...
    numQuestions=int(Questions.val())
    ...
    b0 = Button(
        ...
        borderwidth=0,
        highlightthickness=0,
        command=lambda:[rroot.destroy(),analysis.viewAnalysis(studentName,quizCode),leaderboard.Leaderboard(quizCode)],
        relief="flat")
